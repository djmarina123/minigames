...
abstract final class HubTheme {
  /// Gradiente de fundo do hub (topo → base).
  static const backgroundTop = Color(0xFFFCFCFF);
  static const backgroundMid = Color(0xFFFAF9FD);
  static const backgroundBottom = Color(0xFFF7F5FD);
  /// Cor sólida de fallback (centro do gradiente).
  static const background = backgroundMid;
  ...
  /// Duração padrão de microinterações (cards, botões).
  static const interactionDuration = Duration(milliseconds: 120);
  /// Bottom nav — pulso do ícone ao selecionar aba.
  static const navInteractionDuration = Duration(milliseconds: 150);
  /// Botão de favorito no card — canto inferior direito.
  static const favoriteButtonSize = 32.0;
  /// Pico do pulso do ícone ativo na bottom nav.
  static const navIconSelectedScale = 1.12;
  ...
  /// Botão Remover Ads — gradiente dourado premium.
  static const removeAdsGoldBg = Color(0xFFFFF3D6);
  static const removeAdsGoldMid = Color(0xFFFFE8A8);
  static const removeAdsGoldBorder = Color(0xFFE8B84A);
  ...
  /// Sombra mínima — apenas separa o card do fundo.
  static List<BoxShadow> cardShadow(Color tint, {bool pressed = false}) => [
        BoxShadow(
          color: tint.withValues(alpha: pressed ? 0.08 : 0.045),
          blurRadius: pressed ? 4 : 3,
          offset: Offset(0, pressed ? 1.5 : 1),
        ),
      ];

  /// Sombra discreta no título do card — legibilidade.
  static List<Shadow> cardTitleShadow() => [
        Shadow(
